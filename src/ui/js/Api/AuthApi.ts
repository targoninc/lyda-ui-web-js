import { Api } from "./Api.ts";

export class AuthApi {
    static userExists(email: string, successCallback: Function = () => {
    }, errorCallback: Function = () => {
    }) {
        Api.userExists(email)
            .then(user => user ? successCallback(user) : errorCallback())
            .catch((e) => errorCallback(e));
    }

    static async login(email: string, password: string, challenge: string|undefined, successCallback: Function, errorCallback: Function = () => {}) {
        try {
            const data = await Api.login(email, password, challenge);
            successCallback(data);
        } catch (error) {
            errorCallback(error);
        }
    }

    static async register(username: string, displayname: string, email: string, password: string, userTypes?: string[], links?: { url: string; title?: string }[], successCallback: Function = () => {}, errorCallback: Function = () => {}) {
        try {
            await Api.register(username, displayname, email, password, userTypes, links);
        } catch (e) {
            errorCallback(e);
        }
        successCallback();
    }
}